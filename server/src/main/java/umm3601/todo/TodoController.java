...
public class TodoController implements Controller {
 ...
 // static final String CATEGORY_KEY = "category";
  ...
  private Bson constructFilter(Context ctx) {
    ...
    if (ctx.queryParamMap().containsKey(ROLE_KEY)) {
      String role = ctx.queryParamAsClass(ROLE_KEY, String.class)
        .check(it -> it.matches(ROLE_REGEX), "Todo must have a legal user role")
        .get();
   ...
   * body of the response. The user names and IDs are stored in `TodoIdName` objects,
  ...
  public void getTodosGroupedByCompany(Context ctx) {
    ...
    // (using the `TodoIdName` class to store the user names and IDs).
  ...
  public void addNewTodo(Context ctx) {
    ...
    Todo newTodo = ctx.bodyValidator(Todo.class)
      .check(usr -> usr.name != null && usr.name.length() > 0,
        "Todo must have a non-empty user name; body was " + body)
      .check(usr -> usr.email.matches(EMAIL_REGEX),
        "Todo must have a legal email; body was " + body)
      .check(usr -> usr.age > 0,
        "Todo's age must be greater than zero; body was " + body)
      ...
      .check(usr -> usr.role.matches(ROLE_REGEX),
        "Todo must have a legal user role; body was " + body)
   ...
   * A TodoController instance handles the user endpoints,
  ...
  public void addRoutes(Javalin server) {
    // Get the specified user
    server.get(API_TODO_BY_ID, this::getTodos);
